...
#include <iterator>
#include <functional>
///FAST SUFFIX ARRAY FROM https://judge.yosupo.jp/submission/52300
using namespace std;
/*
...
int main () {
    ...
    blocks.insert(make_pair((int)s.size() - 2, 0));
    int ans = 0;
    int cnt[n + 1];
    for (int i = 0; i <= n; i++) {
        cnt[i] = 0;
    ...
    for (int i = 0; i < n; i++) {
        for (int j: oc[i]) {
            auto p = *blocks.lower_bound(make_pair(j, -1));
            cnt[p.first - p.second + 1]--;
            blocks.erase(p);
            if (j >= p.second) {
                blocks.insert(make_pair(j, p.second));
                cnt[j - p.second + 1]++;
            }
            if (j + 1 <= p.first) {
                blocks.insert(make_pair(p.first, j + 1));
                cnt[p.first - j]++;
            }
        }
